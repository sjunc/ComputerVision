...
09주차 및 10주차 2시간은 총 실습으로 진행  

### Tensorflow 프로그래밍
Tensorflow 프로그래밍의 핵심인 4가지 모듈
• Models API
• Layers API
• Optimizers
• Loss
 ...
 models 모듈  
• 모델 생성하는 models 모듈   
• Sequential과 Functional API가 있음  
• Sequential은 한 갈래 텐서가 끝까지 흐르는 경우에 사용  
• Functional API는 텐서가 여러 갈래로 나뉘는 경우에 사용할 수 있음  
layers, losses, optimizers 모듈  
• 층을 쌓는 layers 모듈  
...
### 손실함수
#### 신경망 초기에는 주로 평균제곱오차(MSE, Mean Squared Error)를 사용  
• 딥러닝에서는 분류를 위해서는 교차 엔트로피(Cross Entropy)를 주로 사용함  
• 이외에도 task의 종류에 따라 다양한 손실 함수를 사용함  
#### cross entropy
- 참값 백터와 예측 백터를 활률 분포로 간주, 로그를 통해서 구함
MSE 와 교차엔트로피 모두 예측이 틀릴 수록 값이 커지는 걸 볼 수 있다.

#### Focal Loss
불균형 데이터(imbalanced data)를 해결하기 위해 Facebook AI 팀이 제안한 손실 함수(2017)  
문제: **객체탐지 문제**에서 대부분의 영역이 배경이며, 일부만 객체임  
• cross entropy loss를 사용하면 학습이 불균형하게 일어남 -> (해결책) 쉽게 맞추는 샘플의 영향은 줄이고, 어려운 샘플에 가중치를 더 주자!(어려운 걸 틀리면 손실이 더큼) -> Focal loss 등장     

### 옵티마이저(Optimizer) 
기본 옵티마이저 SGD를 개선하는 전략으로 모멘텀 적용, 적응적 학습률 적용 등의 방법이 있음  
#### 모멘텀
...
• RMSprop: 그레이디언트를 누적할 때 오래될수록 영향력을 감소하여 AdaGrad 개선  
• Adam: RMSprop에 모멘텀을 적용  
  
### 과적합(Overfitting)과 과소적합(Underfitting
과적합: 일반성을 띄지 않음. 너무 학습데이터에 치중  
과소적합: 학습 데이터, 일반 데이터 모두 낮은 예측률을 보여줌   

### 규제(Regularization)
딥러닝과 머신러닝에서 과적합을 방지하기 위해 모델의 복잡도나 자유도를 제한하는 과정  
일반화 성능을 높임  
다양한 규제 기법  
#### 데이터 증강(Data Augmentation)
• 훈련 집합을 조금씩 변형하여 인위적으로 늘림  
• 오프라인 방식(데이터셋 자체 증강)과 온라인 방식(불러올 때, 주로 온라인 사용)   
#### 드롭아웃(Dropout)
• 학습시 임의의 뉴런 일부를 랜덤하게 꺼 일부는 학습이 되지 않도록 배제함  
...
분류(classification)   
• 영상에 있는 물체가 무엇인지 알아내는 문제.  
• 한 장의 이미지에 대해 하나의 라벨을 예측함.  
• 출력: 클래스 확률 벡터  
• 예: COCO 데이터셋 - 사람, 자전거, 자동차 등 80개의 클래스에 속할 각각의 확률 벡터를 출력함  
종류: 범주 분류, 사례 분류  
• 범주 분류(Categorical Classification)  
• 가장 기본적인 이미지 분류 문제  
• 고양이, 개, 자동차처럼 클래스 자체를 예측함   
• 사례 분류(Instance Classification)    
• crop된 물체의 이미지를 보고, 그 물체가 어떤 class에 속하는지 판별하는 문제  
• 보통 객체 검출/세분화에서 각 인스턴스에 클래스 라벨을 붙이는 단계에서 사용됨   
  
검출(detection)   
• 영상에서 물체를 찾아 직사각형(바운딩 박스)으로 위치 표현  
• 보통 class에 속할 확률도 같이 출력함.  
  
분할(segmentation)  
• 물체가 점유하는 화소(픽셀 단위)를 판별하는 문제. class에 속할 확률도 함께 출력함  
종류: Semantic segmentation, Instance Segmentation   
• Semantic Segmentation   
• 같은 클래스에 속하는 모든 픽셀을 하나의 그룹으로 분류함   
• (예) 모든 '사람' 픽셀을 동일하게 '사람' 클래스로 처리함(의미적으로만 분류)   
• Instance Segmentation  
• 같은 클래스에 속하더라도 서로 다른 물체라면, 서로 다른 instance로 분리함   
• (예) 사람이 3명 있으면, 사람1, 사람2, 사람3으로 따로 구분함(더 발전됨)  
   
추적(tracking)  
• 영상(비디오)에서 특정 물체가 시간에 따라 어떻게 이동하는지 연속적으로 추적하는 문제  
종류: VOT, MOT  
• VOT(Visual Object Tracking) 
• 단일 물체를 추적하는 문제  
• 영상 속에 등장한 하나의 객체를 안정적으로 계속 추적함  
• (예) 특정 선수, 특정 자동차를 추적  
• MOT(Multiple Object Tracking)  
• 다중 물체를 동시에 추적하는 문제  
• 객체마다 ID(고유 번호)를 유지하면서 움직임을 추적함  
• (예) 군중 추적, 여러 차량 추적, CCTV 보행자 추적  
   
행동 분류(Action Recognition)  
• 비디오 속 물체(주로 사람)가 어떤 행동을 하고 있는지 분류하는 문제  
• (예) 스포츠 분석, CCTV 이상행동 감지  

## 객체 검출 문제에서의 성능 측정
### IoU(Intersection over Union)
• : 객체 검출에서 정답 박스와 예측한 박스가 얼마나 겹쳤는지를 수치로 표현한 값  
• IoU = (두 박스가 동시에 겹친 영역의 면적) / (두 박스가 합쳐진 영역의 면적)  
• 일반적으로 정답으로 인정되는 최소 기준으로 IoU 0.5이상 또는 0.75이상을 택함  
...
TP / FP / FN 판정하기  
TP: True Positive
FP: False Positive(가짜를 진짜로 인식)
FN: False Negative(실제 객체를 인식 못함)
• 예측 박스가 여러 개 있을 때, "정확히 맞췄는가?"를 판단해야 함  
• 판단 규칙  
1. 예측 박스를                                                                                                              신뢰도(score) 순으로 정렬함  
2. 위에서부터 하나씩 정답 박스와 비교함  
3. IoU가 임계값보다 크거나 같으면 TP  
• 해당 정답 박스는 더이상 다른 물체와 매칭되지 않음  
4. IoU가 임계값보다 작으면 FP  
5. 예측되지 못한(매칭되지 못한) 정답 박스는 FN  
  
Precision과 Recall 계산  
• 신뢰도 임계값을 바꿔가며 Precision, Recall 계산하여 PR 곡선을 생성함  
• 신뢰도 임계값을 0.0, 0.1, 0.2, ... , 1.0까지 바꿔가며  
** 정밀도(Precision) = TP / (TP+FP) **  
** 재현율(Recall) = TP / (TP+FN) **   
• 임계값을 낮추면 더 많은 박스를 포함하고(Recall 증가, Precision 감소)   
• 임계값을 높이면 더 엄격하게 박스를 선택함(Recall 감소, Precision 증가)    
• 이를 바탕으로 정밀도-재현율 곡선을 생성함(PR Curve)      

PRECISION(정밀도)  
모델이 객체로 인식한 박스 중, 실제 객체가 맞는 비율  
강아지로 예측한 박스가 10개, 그중 실제 강아지가 7개였다면, PRECISION은 0.7  
RECALL(재현율)  
실제로 객체인 것들 중에서 모델이 찾은 객체의 비율  
실제 강아지가 20마리인데, 모델이 그 중 12마리만 잡았다면 recall 은 0.6  

신뢰도 임계값(Confidence Threshold) 자체는 우리가 지정하지만, 그 박스의 신뢰도 값은 모델이 자동으로 계산해줌  
신뢰도: '이 박스안에 객체가 있을 확률'  
신뢰도 임계값: "개발자가 정하는 임계값", 예) 신뢰도가 0.5 이하면 이 박스는 버리시오  


PR 곡선 생성 및 AP(Average Precision) 계산  
• 앞서 구한 (Recall, Precision) 점들을 좌표에 찍어 연결한 그래프가 PR 곡선임  
• 가로축 Recall, 세로축 Precision  
• 이 곡선 아래 면적(Area Under Curve)이 바로 **AP(Average Precision)**가 됨
• 모든 class에 대해 AP를 구한 후, 평균을 내면 mAP(mean Average Precision)가 됨   


mAP 값으로서 성능을 측정함
• mAP(mean Average Precision) 구하는 과정
• 1단계: 예측한 박스 필터링
• 2단계: 예측한 박스를 신뢰도 높은 순서로 정렬함
• 3단계: IoU를 기준으로 TP/FP/FN 판정
• 4단계: 신뢰도 임계값을 0.0부터 1.0까지 변화시키며 PR curve 작성
• 5단계: 각 클래스의 AP 계산
...
• 분류모델과 생성모델  
예  
• 분류 모델: 진짜 데이터와 가짜 데이터를 분류  
• 생성 모델: 새로운 이미지를 만들어냄  
• 생성 모델은 데이터의 분포를 배운 후, 그 분포에서 새로운 샘플을 만들어내는 것  
• 대표적인 생성 모델들  
• Gaussian Mixture Model(GMM), 은닉 마르코프 모델(HMM), 오토인코더, 변분 오토인코더, GAN(Generative Adversarial Network), Diffusion Model 등  

가우시안 기반 생성모델 적용  
• 데이터가 가우시안 분포를 따른다고 가정하면, 평균과 공분산값만 있으면 새로운 샘플을 쉽게 생성 가능함   
공분산(covariance): 두 변수가 함께 어떻게 변하는지를 나타내는 값  
• (예) 키가 클수록 몸무게가 커지면 양(+)의 공분산  
• 시험 점수는 올라가는데, 스트레스는 줄어든다면 음(-)의 공분산  
• 서로 전혀 관계가 없으면 0에 가까운 공분산  

**GaussianMixture(n_components = k)**   

### 오토인코더  Autoencoder
Autoencoder(오토인코더)  
• 입력된 데이터에서 주요 특징을 뽑아 압축하였다(latent vector) 다시 원래대로 복원하는신경망• 입력 -> 주요 특징들(latent vector) -> 다시 복원  
• 데이터들에서 주요 특징들을 뽑아, latent vector가 있는 공간(latent space) 근처에서 적당한점을골라 다시 복원하는 모델에 입력하면 새로운 이미지가 생성됨  
• 즉, 이 구조를 생성 모델처럼 활용 가능함  
단점:  
단순한 오토인코더 구조에서는 latent space에서 적당한 점을 골라 decoder에 입력하면 엉뚱한 이미지가 생성됨   
해결책: 변분 오토인코더  

### 변분 오토인코더 Variational Autoencoder
Variational Autoencoder(변분 오토인코더, VAE)  
• 기존 autoencoder의 latent space에서 latent vector 근처의 점을 다시 복원하면 이상한이미지가나오는 문제를 해결하기 위해 고안된 모델  
• 여기에서는 하나의 고정된 latent vector가 아닌, 확률적 분포(평균 μ, 분산 σ)를 출력하게만들어latent space가 연속적이며 매끄럽게 정렬된 공간으로 나옴  
• 즉, 잘 정렬된 latent space를 만들도록 학습하며, 아무 점을 선택하여 복원하여도 자연스러운이미지가 생성되도록 개선하였음  
• 장점: 생성되는 이미지가 안정적임  
• 단점: VAE로 생성한 이미지가 GAN으로 생성한 이미지보다 흐린 경향이 있음  

### Generative Adversarial Network(GAN, 생성형 적대 신경망)
• 생성자와 판별자가 경쟁하며 점점 정교한 이미지를 만들도록 학습되는 신경망  
• 생성자(G): 위조지폐범 -> 가짜 데이터를 만듦  
• 판별자(D): 경찰 -> 가짜 데이터를 적발함  
• 생성자와 판별자를 경쟁하게 만들면서 둘 다 실력이 점점 향상되도록 만듦  
• 장점: 선명하고, 사실적인 이미지를 생성할 수 있음  
